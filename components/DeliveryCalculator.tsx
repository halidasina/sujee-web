...
export function DeliveryCalculator() {
  ...
  const waMsg = `Salam! Saya nak order Kek Suji Klasik. Alamat penghantaran saya:\n\n${address}\n\nBoleh tolong calculate delivery fee & confirm order?`
      ...
      <div className="max-w-2xl mx-auto px-6">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-serif text-[#5c3d2e] mt-2">
            Order & Semak Delivery Fee
          </h2>
          <p className="text-[#5c3d2e]/70 mt-3 font-sans text-sm">
            Masukkan alamat anda — kami akan calculate delivery fee & confirm terus via WhatsApp
          </p>
        ...
        <div className="bg-[#fdf8f0] rounded-2xl p-8 shadow-sm border border-[#c8973a]/20">
          ...
          <a
            href={address.trim() ? waHref : undefined}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => { if (!address.trim()) e.preventDefault() }}
            className={`mt-4 w-full flex items-center justify-center gap-3 font-sans font-medium py-4 rounded-xl transition-colors duration-200 ${
              address.trim()
          ...
          >
            ...
            Hantar Alamat via WhatsApp
          </a>

          <p className="mt-4 text-center text-xs font-sans text-[#5c3d2e]/50">
            Kami akan balas dengan delivery fee & confirmation dalam masa singkat
          </p>
        </div>
      </div>
    </section>
