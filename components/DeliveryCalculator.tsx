...
export function DeliveryCalculator() {
  ...
  const orderMessage = result?.fee
    ...
    : `Salam! Saya nak tanya delivery fee untuk Kek Suji Klasik. Alamat penghantaran saya: ${address}. Boleh tolong calculate?`
      ...
      <div className="max-w-2xl mx-auto px-6">
        ...
        <div className="bg-[#fdf8f0] rounded-2xl p-8 shadow-sm border border-[#c8973a]/20">
          ...
          {result && (
            <div className="mt-6">
              {result.error ? (
                <div className="text-center p-5 bg-[#fdf8f0] rounded-xl border border-[#c8973a]/30">
                  {result.error.includes('KL') || result.error.includes('Selangor') ? (
                    <>
                      <p className="text-red-500 font-sans text-sm font-medium">{result.error}</p>
                      <p className="text-[#5c3d2e]/60 font-sans text-xs mt-2">
                        Penghantaran hanya untuk KL & Selangor sahaja.
                      </p>
                    </>
                  ) : (
                    <>
                      <p className="text-[#5c3d2e] font-sans text-sm font-medium mb-1">
                        Tanya delivery fee terus via WhatsApp
                      </p>
                      <p className="text-[#5c3d2e]/60 font-sans text-xs mb-4">
                        Kami akan calculate & confirm fee untuk anda dalam masa singkat.
                      </p>
                      <a
                        href={waOrderUrl(orderMessage)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-sans font-medium px-6 py-3 rounded-xl transition-colors duration-200"
                      >
